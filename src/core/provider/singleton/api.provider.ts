...
class ApiProvider<T> extends ApiOperationBase<T> {
  ...
  public async handleHttpRequestResponse(
    ...
    callback: (
      ...
      bodyData?: T | string | Array<string>
    ...
    if (hasParam) {
      ...
      if (bodyData) {
        if (Array.isArray(bodyData)) {
          ...
          await this.methodProvider[method](() =>
            this.apiOperationProvider.execute(() => callback(undefined, bodyData as T))
          );
        } else {
          ...
          if (isValidate && bodyDataErrors.length > 0) {
            ...
            await this.methodProvider[method](() =>
              this.apiOperationProvider.execute(() => callback(undefined, bodyData))
            );
          }
        }
      } else {
        return this.apiOperationProvider.sendErrorResponse({ message: 'bodyData was not defined' });
      }
    } else {
